...
import { getQuizResult } from "../graphql/queries";
import { updateQuizResult } from "../graphql/mutations";
...
export default function QuizResultUpdateForm(props) {
  const {
    id: idProp,
    quizResult: quizResultModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = quizResultRecord
      ? { ...initialValues, ...quizResultRecord }
      : initialValues;
    setUserId(cleanValues.userId);
    setTopic(cleanValues.topic);
    setTotalQuestions(cleanValues.totalQuestions);
    setCorrectAnswers(cleanValues.correctAnswers);
    setScore(cleanValues.score);
    setQuestionsAsked(cleanValues.questionsAsked ?? []);
  ...
  const [quizResultRecord, setQuizResultRecord] =
    React.useState(quizResultModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getQuizResult.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getQuizResult
        : quizResultModelProp;
      setQuizResultRecord(record);
    };
    queryData();
  }, [idProp, quizResultModelProp]);
  React.useEffect(resetStateValues, [quizResultRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateQuizResult.replaceAll("__typename", ""),
            variables: {
              input: {
                id: quizResultRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "QuizResultUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || quizResultModelProp)}
